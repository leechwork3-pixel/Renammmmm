...
async def ban_user(client: Client, message: Message):
    """
    Ban a user by replying to one of their messages.
    """
...
async def unban_user(client: Client, message: Message):
    """
    Unban a user by replying to one of their messages.
    """
    if not message.reply_to_message:
...
async def search_files(client: Client, message: Message):
    """
    Search for files in the dump channel matching a query.
    Returns up to 5 results with document previews.
    """
    if len(message.command) < 2:
        return await message.reply_text("Please provide a search query. Example: `/search filename`")
...
async def leaderboard(client: Client, message: Message):
    """
    Show the top 10 users ordered by queue length in descending order.
    """
